<div id="top"></div>

<br />
<div align="center">

<!-- 🔰 BADGES -->
![GitHub repo size](https://img.shields.io/github/repo-size/codewith-salman/woody-furniture-website)
![GitHub stars](https://img.shields.io/github/stars/codewith-salman/woody-furniture-website?style=social)
![GitHub forks](https://img.shields.io/github/forks/codewith-salman/woody-furniture-website?style=social)

[![Live Demo](https://img.shields.io/badge/Live%20Demo-View%20Project-brightgreen?style=for-the-badge&logo=vercel)](https://woody-furniture-website.vercel.app/)

[![GitHub Follow](https://img.shields.io/github/followers/codewith-salman?style=social)](https://github.com/codewith-salman)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Salman%20Arshad-blue?style=social&logo=linkedin)](https://www.linkedin.com/in/muhammad-salmanarshad/)
[![Figma](https://img.shields.io/badge/Figma-codewithsalman-black?style=social&logo=figma)](https://www.figma.com/@codewithsalman)
[![Email](https://img.shields.io/badge/Email-msalmanwebdev%40gmail.com-red?style=social&logo=gmail)](mailto:msalmanwebdev@gmail.com)

<br />

<h1 align="center">🪵 Woody – Furniture Website</h1>

An elegant and responsive **furniture store landing page**  
designed to showcase modern home décor products with style and clarity.

<a href="https://woody-furniture-website.vercel.app/"><strong>➥ Live Demo</strong></a>

</div>

---

<!-- TABLE OF CONTENTS -->
<details>
  <summary>📑 Table of Contents</summary>
  <ol>
    <li><a href="#about-the-project">About The Project</a></li>
    <li><a href="#features">Features</a></li>
    <li><a href="#built-with">Built With</a></li>
    <li><a href="#contact">Contact</a></li>
  </ol>
</details>

---

## 📖 About The Project

<img src="Woody%20Furniture/assets/images/screenshot.PNG" alt="Woody Furniture Website Screenshot" width="100%" />

**Woody** is a clean, stylish, and conversion-focused landing page created for a furniture and home décor brand.  
The design follows a minimalist aesthetic with smooth visual flow, focusing on premium presentation and usability.

This project includes well-structured sections for:
- Product highlights  
- Collections and offers  
- Testimonials and CTAs  

The UI emphasizes strong visual hierarchy, brand consistency, and responsiveness across all devices.

This is a **front-end UI project**, built for:
- Portfolio showcase  
- UI/UX practice  
- Client demos  
- Furniture & eCommerce landing page concepts  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## ✨ Features

- Elegant hero section with furniture highlights  
- Product and category showcase sections  
- Fully responsive layout (desktop, tablet, mobile)  
- Interactive buttons and hover animations  
- Clean and modern UI principles  
- Ideal for furniture, interior, or eCommerce UI demos  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 🛠️ Built With

This project is built using:

- **HTML5**  
- **CSS3**  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 📬 Contact

**Muhammad Salman Arshad**

- 💼 **LinkedIn:** https://www.linkedin.com/in/muhammad-salmanarshad/  
- 🎨 **Figma:** https://www.figma.com/@codewithsalman  
- 📧 **Email:** [msalmanwebdev@gmail.com](mailto:msalmanwebdev@gmail.com)  

<p align="right">(<a href="#top">back to top</a>)</p>
